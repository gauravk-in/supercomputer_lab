...
int main( int argc, char *argv[] )
{
	...
	while(1) {
		...
		case 0: // JACOBI
		    ...
		    residual = relax_jacobi_return_residual(&param, coords , comm2d);
			tmp = param.u;
			param.u = param.uhelp;
			param.uhelp = tmp;
		    break;
		...
		case 1: // GAUSS
		    if (param.thread_dims[0] == 1)
		    	residual = relax_gauss_return_residual_1dy(&param, param.maxiter, coords , comm2d);
		    else if (param.thread_dims[1] == 1)
                        residual = relax_gauss_return_residual_1dx(&param, param.maxiter, coords , comm2d);
		    else 
			residual = relax_gauss_return_residual_2d(&param, param.maxiter, coords , comm2d);
		    break;
	    }
	    if (param.algorithm == 0) // JACOBI
		iter += 1;
	    else // GAUSS  
	    	iter+= param.maxiter;
